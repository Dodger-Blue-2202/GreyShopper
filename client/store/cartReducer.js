...
export const removeFromCart = (product) => async dispatch => {
  const token = window.localStorage.getItem(TOKEN)
    try{
        if (token) {
            await axios.delete(`/api/orders/products`,{headers:{authorization:token,body:product.id}})
        }
...
export const addToCart = (product) => async dispatch => {
    const token = window.localStorage.getItem(TOKEN)
      try{
          if (token) {
              await axios.post(`/api/orders/products`,{headers:{authorization:token,body:product.id}})
          }
...
export const checkOut = (order) => async dispatch => {
  ...
  //VERIFY AUTHORIZATION WITH TOKEN
    try{
      await axios.put(`/api/orders/checkout`,{headers:{authorization:token},body:order}) // if token exists in authorization then checkout order by setting isCart to false identified by token
      ...
      dispatch(setError("Error getting order data. Current user might not have admin access"))
    }
  }
  ...
  export const signUp = (order) => async dispatch =>{
    const token = window.localStorage.getItem(TOKEN)
      try{
        await axios.post(`/api/orders/`,{headers:{authorization:token},body:order}) // signing up will add to the db all the items in your cart
      }catch(err){
        dispatch(setError("Error getting order data. Current user might not have admin access"))
      }
    }
...
export default function(state = [], action) {
  switch (action.type) {
    ...
    case REMOVE_FROM_ORDER:
      return state.filter((item)=>{return(item.id!==action.product.id)})
    case ADD_TO_ORDER:
