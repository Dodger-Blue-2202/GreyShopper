...
export const deleteProduct = (id) => async dispatch => {
  ...
  if (token) {
    try{
      const res =  await axios.delete(`/api/products/:${id}`,{headers:{authentication:token}})
      //deletes product if we have admin access
      dispatch(removeProduct(res.data.id)) 
      //update store
    }catch(err){
      dispatch(setError("Error getting product data. Current user might not have admin access"))
    }
  }else {
...
export const postProduct = (product) => async dispatch => {
  ...
  if (token) {
    try{
      const res =  await axios.post(`/api/products/`,{headers:{authentication:token},body:product})
...
export const putProduct = (product) => async dispatch => {
  ...
  if (token) {
    try{
      const res =  await axios.put(`/api/products/:${product.id}`,{headers:{authentication:token},body:product})
      //adds product if we have admin access
...
export default function(state = [], action) {
  switch (action.type) {
    ...
    case EDIT_PRODUCT:
      return state.map((item)=>{if (item.id===action.product.id){return (action.product)}else{return item}})
      default:
      return state
  }
}
